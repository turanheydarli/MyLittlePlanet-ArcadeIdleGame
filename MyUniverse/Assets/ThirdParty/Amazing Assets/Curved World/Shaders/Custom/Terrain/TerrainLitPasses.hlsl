...
Varyings SplatmapVert(Attributes v)
{
    ...
    TerrainInstancing(v.positionOS, v.normalOS, v.texcoord);
      ...
      float4 tangent = float4(cross(v.normalOS, float3(0, 0, 1)), -1);
      CURVEDWORLD_TRANSFORM_VERTEX_AND_NORMAL(v.positionOS, v.normalOS, tangent)
   #else
...
VaryingsLean ShadowPassVertex(AttributesLean v)
{
   ...
   #ifdef CURVEDWORLD_NORMAL_TRANSFORMATION_ON
      float4 tangent = float4(cross(v.normalOS, float3(0, 0, 1)), -1);
      CURVEDWORLD_TRANSFORM_VERTEX_AND_NORMAL(v.position, v.normalOS, tangent)
   #else
