...
namespace AmazingAssets.CurvedWorldEditor
{
    public static class EditorUtilities
    {
        ...
        static Dictionary<int, BendFiles> projectBendsFilePathes = null;
        ...
        static public string GetBendFileLocation(CurvedWorld.BEND_TYPE bendType, int bendID, EXTENSTION extention)
        {
            ...
            {
                if (projectBendsFilePathes != null)
                    projectBendsFilePathes.Clear();

                projectBendsFilePathes = new Dictionary<int, BendFiles>();
            }

            bendID = (int)Mathf.Clamp(bendID, 1, EditorUtilities.MAX_SUPPORTED_BEND_IDS);

            int key = (int)bendType * EditorUtilities.MAX_SUPPORTED_BEND_IDS + bendID;


            if (projectBendsFilePathes.ContainsKey(key) == false)
            {
                BendFiles bendFiles;
                bendFiles.pathToCGINC = GetGeneratedFilePath(bendType, bendID, EXTENSTION.cginc, false);
                bendFiles.pathToUnityShaderGraphNormal = GetGeneratedFilePath(bendType, bendID, EXTENSTION.UnityShaderGraphNormal, false);
                bendFiles.pathToUnityShaderGraphVertex = GetGeneratedFilePath(bendType, bendID, EXTENSTION.UnityShaderGraphVertex, false);
                bendFiles.pathToAmplifyShaderEditorNormal = GetGeneratedFilePath(bendType, bendID, EXTENSTION.AmplifyShaderEditorNormal, false);
                bendFiles.pathToAmplifyShaderEditorVertex = GetGeneratedFilePath(bendType, bendID, EXTENSTION.AmplifyShaderEditorVertex, false);

                projectBendsFilePathes.Add(key, bendFiles);
            }
            ...
            switch (extention)
            {
                case EXTENSTION.cginc:
                    return projectBendsFilePathes[key].pathToCGINC;

                case EXTENSTION.UnityShaderGraphNormal:
                    return projectBendsFilePathes[key].pathToUnityShaderGraphNormal;
                case EXTENSTION.UnityShaderGraphVertex:
                    return projectBendsFilePathes[key].pathToUnityShaderGraphVertex;

                case EXTENSTION.AmplifyShaderEditorNormal:
                    return projectBendsFilePathes[key].pathToAmplifyShaderEditorNormal;
                case EXTENSTION.AmplifyShaderEditorVertex:
                    return projectBendsFilePathes[key].pathToAmplifyShaderEditorVertex;
        ...
        static public void RebuildProjectBendsInfo()
        {
            projectBendsFilePathes = null;
        ...
        static public ACTIVATE_STATE ActivateShader(Shader shader, bool activate, bool reimport)
        {
            if (shader == null)
                return ACTIVATE_STATE.Skip;

            if (IsShaderBuiltInResource(shader))
                return ACTIVATE_STATE.Skip;
            ...
            if (hasProperty == false || hasBendType == false || hasBendID == false || hasPathtoCGINC == false)
            {
                string warningMessage = string.Format("Curved World {0} problem for shader '{1}'\nhasProperty: {2}\nhasBendType: {3}\nhasBendID: {4}\nhasPathtoCGINC: {5}\n", (activate ? "activation" : "deactivation"), shader.name, hasProperty, hasBendType, hasBendID, hasPathtoCGINC);
                Debug.LogWarning(warningMessage, shader);
        ...
        public struct BendFiles
        {
            public string pathToCGINC;

            public string pathToUnityShaderGraphNormal;
            public string pathToUnityShaderGraphVertex;

            public string pathToAmplifyShaderEditorNormal;
            public string pathToAmplifyShaderEditorVertex;
        }
