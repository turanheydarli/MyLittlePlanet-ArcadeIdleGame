...
namespace AmazingAssets.CurvedWorldEditor
{
    public class CurvedWorldEditorWindow : EditorWindow
    {
        ...
        void Draw_AboutTab()
        {
            ...
            using (new EditorGUIHelper.GUIEnabled(File.Exists(manualFilePass)))
            {
                if (GUILayout.Button(new GUIContent("Documentation", iconManual), GUILayout.MaxHeight(buttonHeight), GUILayout.Width(buttonWidth)))
                {
                    if (File.Exists(manualFilePass))
                    {
                        manualFilePass = manualFilePass.Replace('/', Path.DirectorySeparatorChar);

                        System.Diagnostics.Process fileopener = new System.Diagnostics.Process();
                        fileopener.StartInfo.FileName = "explorer";
                        fileopener.StartInfo.Arguments = "\"" + manualFilePass + "\"";
                        fileopener.Start();
                    }
                    else
                    {
                        Debug.Log("File '" + manualFilePass + "' does not exist.\n");
                    }
                }
            }
